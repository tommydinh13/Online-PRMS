...
public class RegisterUserForm implements ActionListener {
	...
	RegisterUserForm() {
		...
		emailLabel.setBounds(50, 40, 100, 20); // where the label will go on the panel (x,y,width,height)
		...
		passwordLabel.setBounds(50, 70, 100, 20); // where the label will go on the panel (x,y,width,height)
		frame.add(passwordLabel);

		passwordText = new JPasswordField(20); // creating box that lets user enter chars that are hidden
		passwordText.setBounds(160, 70, 165, 25); // setting bounds (x,y,width,height)
		frame.add(passwordText);

		confirmPassLabel = new JLabel("Confirm Password"); // label that goes beside Password to tell user what to enter
		confirmPassLabel.setBounds(50, 100, 130, 20); // where the label will go on the panel (x,y,width,height)
		frame.add(confirmPassLabel);

		confirmPassText = new JPasswordField(20); // creating box that lets user enter chars that are hidden
		confirmPassText.setBounds(160, 100, 165, 25); // setting bounds (x,y,width,height)
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == signupButton) {
			...
			if (email.equals("Tommy") && checkPassword(password, confirmPass)) {
				...
				JOptionPane.showMessageDialog(null, email + " Already Exists!", "Email Error",
